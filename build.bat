...
:: --- Usage Notes -------------------------------------------------------
::
:: Central build script for FTP-FTW. Single unity-build translation
...
:: /Z7 (MSVC) / -gcodeview (clang) is debug-only, not in the shared
:: _common flags below - a release build should carry no embedded debug
:: info (source line/type data, absolute build-machine paths) into the
:: shipped .exe at all. /guard:cf (Control Flow Guard) and the explicit
:: DYNAMICBASE/NXCOMPAT/HIGHENTROPYVA link flags below are release-only
:: hardening (cheap defense-in-depth against exploitation of any
:: as-yet-unknown memory-safety bug) - CFG is MSVC-only here since plain
:: clang (as opposed to clang-cl) + lld-link doesn't reliably support it.
set cl_common=  /I..\src\ /nologo /FC /Zc:preprocessor /W3
set cl_debug=   call cl /Od /Ob1 /Z7 /DBUILD_DEBUG=1 %cl_common%
set cl_release= call cl /O2 /guard:cf /DBUILD_DEBUG=0 %cl_common%
set cl_link=    /link /INCREMENTAL:NO /opt:ref /DYNAMICBASE /NXCOMPAT /HIGHENTROPYVA
set cl_link_release= %cl_link% /guard:cf
set cl_out=     /out:
set cl_obj_out= /Fo:
set cl_only_compile= /c
set cl_mklib=   call lib -nologo

set clang_common=  -mcx16 -I..\src\ -fdiagnostics-absolute-paths -Wall -Wno-unused-function
set clang_debug=   call clang -g -gcodeview -O0 -DBUILD_DEBUG=1 %clang_common%
set clang_release= call clang -O2 -DBUILD_DEBUG=0 %clang_common%
set clang_link=    -fuse-ld=lld -Xlinker /INCREMENTAL:NO -Xlinker /opt:ref -Xlinker /DYNAMICBASE -Xlinker /NXCOMPAT -Xlinker /HIGHENTROPYVA
set clang_out=     -o
set clang_obj_out= -o
...
if "%msvc%"=="1" if "%release%"=="1" set compile_link=%cl_link_release%
...
:: --- Version -------------------------------------------------------------
::
:: Stamped into the .exe (see src/version.h) from the FTW_VERSION env var,
:: which the release workflow sets from the pushed git tag. Not set for a
:: normal local build - version.h's own fallback ("0.0.0-dev") applies and
:: version_define stays empty. /D vs -D differs between MSVC and clang, same
:: as every other flag pair above.
set version_define=
if not "%FTW_VERSION%"=="" (
  echo [version %FTW_VERSION%]
  if "%msvc%"=="1"  set version_define=/DFTW_VERSION=\"%FTW_VERSION%\"
  if "%clang%"=="1" set version_define=-DFTW_VERSION=\"%FTW_VERSION%\"
)
...
echo [building ftp-ftw]
%compile% -I%libssh2_dir%\include %version_define% ..\src\main.c fp_dwrite.obj %compile_link% %libssh2_libs% %gui_libs% %out%ftp-ftw.exe
if %errorlevel% neq 0 (popd & exit /b 1)
popd

echo [ftp-ftw build succeeded]
