...
internal B32
fs_dir_create(String8 path)
{
  Temp scratch = scratch_begin(0, 0);
  WCHAR *wide = fs__wide_from_path(scratch.arena, path);
  // Walk left to right, creating each intermediate directory in turn (past
  // the drive-root separator, e.g. "C:\") - CreateDirectoryW on one that
  // already exists just fails with ERROR_ALREADY_EXISTS, harmless to
  // ignore here since only the final, full-path result matters.
  for(U64 i = 0; wide[i] != 0; i += 1)
  {
    if(wide[i] == L'\\' && i > 2)
    {
      wide[i] = 0;
      CreateDirectoryW(wide, 0);
      wide[i] = L'\\';
    }
  }
  BOOL ok = CreateDirectoryW(wide, 0);
  DWORD err = GetLastError();
  scratch_end(scratch);
  return ok != 0 || err == ERROR_ALREADY_EXISTS;
}
...
internal FS_File
fs_file_open_write_keep(String8 path)
{
  Temp scratch = scratch_begin(0, 0);
  WCHAR *wide = fs__wide_from_path(scratch.arena, path);
  HANDLE h = CreateFileW(wide, GENERIC_WRITE, 0, 0, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
  scratch_end(scratch);
  FS_File result = {(U64)h};
  return result;
}
...
internal B32
fs_file_seek(FS_File file, U64 offset)
{
  HANDLE h = (HANDLE)PtrFromInt(file.u64[0]);
  LARGE_INTEGER li;
  li.QuadPart = (LONGLONG)offset;
  return SetFilePointerEx(h, li, 0, FILE_BEGIN) != 0;
}
