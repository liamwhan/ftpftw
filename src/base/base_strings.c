...
str8fv(Arena *arena, char *fmt, va_list args)
{
  va_list args2;
  va_copy(args2, args);
  int needed_signed = vsnprintf(0, 0, fmt, args);
  // A negative return (encoding error) cast straight to U64 wraps to
  // ~max_U64, turning one malformed format call - including ones
  // formatting server-supplied strings, e.g. an error message with a
  // stray invalid byte - into an ~18-exabyte allocation request that
  // aborts the whole process (see arena_push_/arena_alloc_'s
  // abort_self(1) on reserve failure). Fall back to an empty string
  // instead of ever handing arena_push a wrapped-negative size.
  U64 needed = (needed_signed >= 0) ? ((U64)needed_signed + 1) : 1;
  U8 *buffer = push_array_no_zero(arena, U8, needed);
  U64 actual = (needed_signed >= 0) ? (U64)vsnprintf((char *)buffer, needed, fmt, args2) : 0;
  if(needed_signed < 0)
  {
    buffer[0] = 0;
  }
