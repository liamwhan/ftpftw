...
// Polled right mouse button down-state, same convention as
// wm_mouse_state's left-button field - a separate query rather than
// folded into wm_mouse_state so most callers (which only ever care about
// the left button) don't need an unused out-param. No mouse-capture
// tracking on this button (unlike left) - a context menu doesn't need the
// up-event delivered outside the window, so this stays a plain state poll.
internal B32 wm_mouse_right_down(void);
...
// Cursor shapes the app can ask for over the client area - just the ones
// our own resize handles need (see wm_set_cursor below), not a general
// cursor-theming API.
typedef U32 WM_CursorKind;
enum
{
  WM_CursorKind_Arrow,
  WM_CursorKind_SizeWE, // horizontal double-headed arrow - local/remote pane divider
  WM_CursorKind_SizeNS, // vertical double-headed arrow - bottom-pane divider
};

// Sets which cursor shape WM_SETCURSOR should apply over the client area -
// call once per frame with whatever's appropriate for the current hover/
// drag state (defaulting to WM_CursorKind_Arrow), same polled-once-per-
// frame convention as wm_mouse_state. The window intercepts WM_SETCURSOR
// itself to apply this rather than letting Win32 reset to the class cursor
// on every WM_MOUSEMOVE, which is what a plain SetCursor call here would
// otherwise fight against.
internal void wm_set_cursor(WM_CursorKind kind);
