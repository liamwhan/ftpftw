...
global B32 wm_g_mouse_right_down = 0;
global F32 wm_g_wheel_delta = 0.0f;
global WM_CursorKind wm_g_cursor_kind = WM_CursorKind_Arrow;
...
wm_wndproc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
  ...
  switch(msg)
  {
    ...
    case WM_RBUTTONDOWN:
    {
      wm_g_mouse_right_down = 1;
    }break;

    case WM_RBUTTONUP:
    {
      wm_g_mouse_right_down = 0;
    }break;

    case WM_SETCURSOR:
    {
      // Only override the cursor over the client area (LOWORD(lparam) is
      // the hit-test code from the preceding WM_NCHITTEST) - resize
      // borders/title bar etc. outside it should keep Win32's own cursors,
      // so those fall through to DefWindowProc below.
      if(LOWORD(lparam) == HTCLIENT)
      {
        LPCWSTR id = (LPCWSTR)IDC_ARROW;
        switch(wm_g_cursor_kind)
        {
          default: break;
          case WM_CursorKind_SizeWE: { id = (LPCWSTR)IDC_SIZEWE; }break;
          case WM_CursorKind_SizeNS: { id = (LPCWSTR)IDC_SIZENS; }break;
        }
...
wm_window_open(String8 title, U32 width, U32 height)
{
  ...
  if(!wm_g_class_registered)
  {
    ...
    wc.lpszClassName = L"ftp_ftw_wm_window";
    RegisterClassExW(&wc);
  ...
  HWND hwnd = CreateWindowExW(0, L"ftp_ftw_wm_window", wide_title,
                               WS_OVERLAPPEDWINDOW,
...
internal B32
wm_mouse_right_down(void)
{
  return wm_g_mouse_right_down;
}

internal void
wm_set_cursor(WM_CursorKind kind)
{
  wm_g_cursor_kind = kind;
}
...
wm_clipboard_get_text(U8 *out_buf, U64 out_buf_cap)
{
  ...
  if(OpenClipboard(wm_g_hwnd))
  {
    ...
    if(h != 0)
    {
      ...
      if(wide != 0)
      {
        // Bound wcslen by the handle's actual committed size before
        // trusting it to be null-terminated within bounds - the clipboard
        // owner is some other process (any other app on the system, not
        // just this one), and a CF_UNICODETEXT block that's undersized or
        // missing its terminator would otherwise read past the allocation.
        U64 global_size = (U64)GlobalSize(h);
        U64 max_wchars_in_block = global_size / sizeof(WCHAR);
        int wide_len = (int)Min(wcsnlen(wide, max_wchars_in_block), 1024);
        char temp[4096];
