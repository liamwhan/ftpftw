internal B32
ui_conn_modal__button(FP_Font *font, F32 x, F32 y, F32 w, F32 h, String8 label)
{
  ...
  F32 advance = 0.0f;
  {
    // center the label - measure first via fnt_text_pieces without drawing,
    // same trick ui_text_edit uses for its cursor position.
                      ...
                      String8 *out_host, U16 *out_port, String8 *out_user, String8 *out_pass)
{
  ...
  if(ui_conn_modal__button(font, btn_x, btn_y, btn_w, btn_h, str8_lit("New")))
  {
    modal->selected_index = -1;
  ...
  if(ui_conn_modal__button(font, btn_x, btn_y, btn_w, btn_h, str8_lit("Save")))
  {
  ...
  if(ui_conn_modal__button(font, btn_x, btn_y, btn_w, btn_h, str8_lit("Delete")))
  {
    if(modal->selected_index >= 0 && (U64)modal->selected_index < modal->profiles.count)
    {
  ...
  if(ui_conn_modal__button(font, btn_x, btn_y, btn_w, btn_h, str8_lit("Connect")))
  {
  ...
  if(ui_conn_modal__button(font, btn_x, btn_y, btn_w, btn_h, str8_lit("Close")))
  {
    modal->open = 0;
  }
